...
from ..formats.skd_format import (
    ...
    SKD_HEADER_V6_SIZE, SKD_HEADER_V5_SIZE, SKD_HEADER_V5_FORMAT,
    ...
    def _write_file(self, surfaces: List[Dict], 
        ...
        for surface in surfaces:
            ...
            for vert in surface['vertices']:
                ...
                # Only include morphs for V6+
                morphs = vert['morphs'] if is_v6 else []
                verts_size += len(morphs) * SKD_MORPH_SIZE  # 16 bytes each
        ...
        for i, surf_info in enumerate(surface_infos):
            ...
            tri_bytes = b''.join([struct.pack('<3i', *tri) for tri in surf_info['triangles']])
            output.write(tri_bytes)
