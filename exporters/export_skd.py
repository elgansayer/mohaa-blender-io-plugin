...
from ..formats.skd_format import (
    ...
    SKD_HEADER_V6_SIZE, SKD_HEADER_V5_SIZE,
...
class SKDExporter:
    ...
    def __init__(self, filepath: str,
        ...
        Args:
            ...
            version: SKD version to export (default: 6)
        """
    ...
    def _write_file(self, surfaces: List[Dict], 
        ...
        for surface in surfaces:
            ...
            # Surface header: 100 bytes (was 84 in comment but code uses 100)
        ...
        # Write header
        # Create SKDHeader object and use its write method
        header = SKDHeader(
            ident=SKD_IDENT_INT,
        ...
        for i, surf_info in enumerate(surface_infos):
            ...
            # Create SKDSurface object and use its write method
            surf_header = SKDSurface(
                ident=0,
                name=surf_info['name'],
                ...
                ofs_verts=ofs_verts,
                ofs_collapse=ofs_collapse,
                ofs_end=ofs_end,
                ofs_collapse_index=ofs_collapse_idx
            )
            surf_header.write(output)
            ...
            for vert in surf_info['vertices']:
                # Construct Vertex object and use write method
                # Convert dict data to objects
                
                morphs = [
                    SKDMorph(m['morph_index'], m['offset'])
                    for m in vert['morphs']
                ]
                
                weights = [
                    SKDWeight(w['bone_index'], w['bone_weight'], w['offset'])
        ...
        for bone_name, parent_name, offset in bones:
            ...
            bone_data = struct.pack(
                ...
                84,  # ofsBaseData (point to immediate data?) 
                     # Standard files usually have boneType 1 (POSROT) with offset data
