...
class SKDExporter:
    ...
    def __init__(self, filepath: str,
        ...
        Args:
            ...
            version: SKD file version (5 or 6)
        """
    ...
    def _write_file(self, surfaces: List[Dict], 
        ...
        header_size = SKD_HEADER_V6_SIZE if self.version >= SKD_VERSION_CURRENT else SKD_HEADER_V5_SIZE
        ...
        for surface in surfaces:
            ...
            verts_size = 0
            # V5: Skip morphs if present in data but not supported?
            # formats/skd_format.py says "When exporting SKD Version 5, the exporter automatically skips morph target data"
        ...
        # Create and write header using SKDHeader
        header = SKDHeader(
            ident=SKD_IDENT_INT,
        ...
        for i, surf_info in enumerate(surface_infos):
            ...
            # Optimization: Bulk write triangles
            ...
            vertex_struct = struct.Struct('<3f 2f i i')
            ...
            for vert in surf_info['vertices']:
                ...
                # Check version for morphs
                if self.version >= SKD_VERSION_CURRENT:
                    num_morphs = len(vert['morphs'])
                else:
                    num_morphs = 0

                vert_chunks.append(vertex_struct.pack(
                ...
                if self.version >= SKD_VERSION_CURRENT:
                    for morph in vert['morphs']:
                        vert_chunks.append(morph_struct.pack(
                            morph['morph_index'],
                ...
                for weight in vert['weights']:
                    vert_chunks.append(weight_struct.pack(
                        weight['bone_index'],
                        weight['bone_weight'],
                        weight['offset'][0], weight['offset'][1], weight['offset'][2]
                    ))
            
            output.write(b''.join(vert_chunks))

            # Write collapse map (identity for now) - Optimize
        ...
        bone_chunks = []
        bone_struct = struct.Struct('<32s 32s i i i i i')
        offset_struct = struct.Struct('<3f')

        for bone_name, parent_name, offset in bones:
            bone_chunks.append(bone_struct.pack(
            ...
            bone_chunks.append(offset_struct.pack(offset[0], offset[1], offset[2]))

        output.write(b''.join(bone_chunks))
