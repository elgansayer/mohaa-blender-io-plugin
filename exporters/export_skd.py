...
from ..formats.skd_format import (
    ...
    SKD_HEADER_V5_SIZE, SKD_HEADER_V6_SIZE, SKD_SURFACE_SIZE, SKD_VERTEX_SIZE,
...
class SKDExporter:
    ...
    def _write_file(self, surfaces: List[Dict], 
        ...
        # Write header
        if self.version >= 6:
            # V6 Format (includes morph targets and scale)
            header_data = struct.pack(
                '<i i 64s i i i i i 10i i i i i f',
                SKD_IDENT_INT,  # ident
                self.version,  # version (usually 6)
            ...
            # V5 Format (no morph targets, no scale)
            header_data = struct.pack(
                '<i i 64s i i i i i 10i i i',
                SKD_IDENT_INT,  # ident
                self.version,  # version (usually 5)
                model_name.encode('latin-1').ljust(64, b'\x00'),  # name
                len(surfaces),  # numSurfaces
                len(bones),  # numBones
                ofs_bones,  # ofsBones
