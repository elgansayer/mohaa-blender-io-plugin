...
class SKDExporter:
    ...
    def _write_file(self, surfaces: List[Dict], 
        ...
        for i, surf_info in enumerate(surface_infos):
            ...
            # Write triangles (Bulk write)
            output.write(b''.join([struct.pack('<3i', *tri) for tri in surf_info['triangles']]))
            ...
            for vert in surf_info['vertices']:
                ...
                num_morphs = len(vert['morphs']) if self.version >= SKD_VERSION_CURRENT else 0
                
                vert_chunks.append(struct.pack(
                    '<3f 2f i i',
            ...
            # Write collapse map (identity for now) - Bulk write
            num_verts = len(surf_info['vertices'])
            output.write(b''.join([struct.pack('<i', v) for v in range(num_verts)]))

            # Write collapse index (identity for now) - Bulk write
            output.write(b''.join([struct.pack('<i', v) for v in range(num_verts)]))
        
        # Write bones
        bone_chunks = []
        for bone_name, parent_name, offset in bones:
            bone_chunks.append(struct.pack(
                '<32s 32s i i i i i',
            ...
            bone_chunks.append(struct.pack('<3f', *offset))
