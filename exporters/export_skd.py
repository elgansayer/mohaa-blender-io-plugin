...
from ..formats.skd_format import (
    ...
    SKD_HEADER_V6_SIZE, SKD_HEADER_V5_SIZE, SKD_HEADER_V6_FORMAT, SKD_HEADER_V5_FORMAT,
...
class SKDExporter:
    ...
    def __init__(self, filepath: str,
        ...
        Args:
            ...
            version: SKD version to export (default: SKD_VERSION_CURRENT)
        """
    ...
    def _write_file(self, surfaces: List[Dict], 
        ...
        if self.version >= SKD_VERSION_CURRENT:
            header_size = SKD_HEADER_V6_SIZE  # 152 bytes
        else:
            header_size = SKD_HEADER_V5_SIZE  # 140 bytes
        ...
        for surface in surfaces:
            ...
            # Surface header: 84 bytes (SKD_SURFACE_SIZE=100 actually defined in format?)
            # Wait, formats/skd_format.py says SKD_SURFACE_SIZE = 100
            ...
            # Calculate exact size for all vertices to optimize offsets
            verts_size = 0
            for vert in surface['vertices']:
                verts_size += SKD_VERTEX_SIZE  # 28 bytes actually in format def?
                # Check format def: 28 bytes. 3f(12)+2f(8)+i(4)+i(4)=28.
        ...
        for i, surf_info in enumerate(surface_infos):
            ...
            # Write surface header (SKD_SURFACE_SIZE)
            ...
            # Write triangles (Batch write)
            # SKD_TRIANGLE_FORMAT = '<3i'
            output.write(b''.join(
                struct.pack('<3i', t[0], t[1], t[2])
                for t in surf_info['triangles']
            ))

            # Write vertices (Batch write)
            # Collect all vertex bytes
            vertex_bytes_list = []
            
            for vert in surf_info['vertices']:
                # Vertex header (28 bytes)
                num_weights = len(vert['weights'])
                num_morphs = len(vert['morphs'])
                
                vertex_bytes_list.append(struct.pack(
                    '<3f 2f i i',
            ...
            output.write(b''.join(vertex_bytes_list))
            
            # Write collapse map (identity for now) - Batch write
            # Just pack range(num_verts)
            output.write(b''.join(
                struct.pack('<i', v) for v in range(num_verts)
            ))
            
            # Write collapse index (identity for now) - Batch write
            output.write(b''.join(
                struct.pack('<i', v) for v in range(num_verts)
            ))
        ...
        output.write(b''.join(
            struct.pack(
                ...
                offset[0], offset[1], offset[2] # extra data
            )
