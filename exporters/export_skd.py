...
class SKDExporter:
    ...
    def _build_surface(self, mesh: bpy.types.Mesh, face_indices: List[int],
        ...
        for face_idx in face_indices:
            ...
            for loop_idx in poly.loop_indices:
                ...
                # V5 doesn't support morphs, but we keep the structure flexible
                vertex = {
                    'normal': normal,
                    'tex_coords': uv,
                    'weights': weights,
    ...
    def _write_file(self, surfaces: List[Dict], 
        ...
        is_v5 = (self.version == SKD_VERSION_OLD)
        ...
        # Pre-allocate optimizers
        tri_packer = struct.Struct('<3i').pack
        vert_header_packer = struct.Struct('<3f 2f i i').pack
        morph_packer = struct.Struct('<i 3f').pack
        weight_packer = struct.Struct('<i f 3f').pack
        int_packer = struct.Struct('<i').pack
        ...
        for i, surf_info in enumerate(surface_infos):
            ...
            for tri in surf_info['triangles']:
                packed_tris.append(tri_packer(tri[0], tri[1], tri[2]))
            output.write(b''.join(packed_tris))
            ...
            for vert in surf_info['vertices']:
                ...
                num_morphs = 0 if is_v5 else len(vert['morphs'])
                
                packed_verts.append(vert_header_packer(
                ...
                # Write morphs (16 bytes each) - Skip if V5
                if not is_v5:
                    for morph in vert['morphs']:
                        packed_verts.append(morph_packer(
                            morph['morph_index'],
                ...
                for weight in vert['weights']:
                    packed_verts.append(weight_packer(
                        weight['bone_index'],
                        weight['bone_weight'],
                        weight['offset'][0], weight['offset'][1], weight['offset'][2]
                    ))
            output.write(b''.join(packed_verts))
            
            # Write collapse map (identity for now)
            packed_collapse = [int_packer(v) for v in range(num_verts)]
            output.write(b''.join(packed_collapse))
            
            # Write collapse index (identity for now)
            # Same content as collapse map in this implementation
            output.write(b''.join(packed_collapse))
        
        # Write bones (using boneFileData_t format: 84 bytes each)
        packed_bones = []
        bone_packer = struct.Struct('<32s 32s i i i i i').pack
        offset_packer = struct.Struct('<3f').pack

        for bone_name, parent_name, offset in bones:
            # We don't have separate base data or channel names, so offsets are 0
            packed_bones.append(bone_packer(
            ...
            packed_bones.append(offset_packer(offset[0], offset[1], offset[2]))

        output.write(b''.join(packed_bones))
