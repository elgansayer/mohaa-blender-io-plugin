...
class SKCExporter:
    ...
    def _write_animation(self, action: bpy.types.Action) -> None:
        ...
        # Optimization: Build frame headers in bulk
        ...
        for i, frame in enumerate(frame_data):
            ...
            frame_chunks.append(frame_struct.pack(
        ...
        output.write(b''.join(frame_chunks))

        # Write channel data for all frames
        # Optimization: Bulk write channel data
        channel_struct = struct.Struct('<4f')
        channel_chunks = []

        for frame_channels in channel_values:
            for channel_data in frame_channels:
                channel_chunks.append(channel_struct.pack(*channel_data))

        output.write(b''.join(channel_chunks))
        
        # Write channel names
        name_chunks = []
        for channel_name, _ in channels:
            name_chunks.append(channel_name.encode('latin-1')[:32].ljust(32, b'\x00'))

        output.write(b''.join(name_chunks))
