...
class SKCExporter:
    ...
    def __init__(self, filepath: str,
        ...
        Args:
            ...
            version: SKC version to export (13 or 14)
        """
    ...
    def _write_animation(self, action: bpy.types.Action) -> None:
        ...
        # Bulk write channel data for all frames
        # Use a flat list comprehension and pack all at once if possible,
        # or join packed bytes

        # Flatten all channel data: [x, y, z, w, x, y, z, w, ...]
        # Itertools chain would be good here, but list comprehension is fine
        flat_channel_data = [
            val
        ...
        # Format string will be '<Nf' where N is number of floats
        if flat_channel_data:
            num_floats = len(flat_channel_data)
            output.write(struct.pack(f'<{num_floats}f', *flat_channel_data))
